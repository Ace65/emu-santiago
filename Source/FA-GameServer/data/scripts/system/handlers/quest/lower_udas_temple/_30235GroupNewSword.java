	...
	public HandlerResult onItemUseEvent(final QuestCookie env, Item item)
	{
		...
		if(id == 186000107)
		{
			...
			if(qs.getQuestVarById(0) != 0)
				return HandlerResult.FAILED;
			if(player.getInventory().getItemCountByItemId(100000939) == 0 || player.getInventory().getItemCountByItemId(186000099) == 0
			...
			ThreadPoolManager.getInstance().schedule(new Runnable(){
				...
				public void run()
				{
					ItemService.addItems(player, Collections.singletonList(new QuestItems(182209633, 1)));
