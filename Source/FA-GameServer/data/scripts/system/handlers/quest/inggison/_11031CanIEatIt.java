...
public class _11031CanIEatIt extends QuestHandler
{
	...
	public void register()
	{
		...
		qe.setQuestItemIds(182206724).add(questId);
	}
	...
	public HandlerResult onItemUseEvent(final QuestCookie env, Item item)
	{
		...
		ThreadPoolManager.getInstance().schedule(new Runnable(){
			...
			public void run()
			{
				player.getInventory().removeFromBagByItemId(182206724, 1);	
	...
	public boolean onDialogEvent(QuestCookie env)
	{
		...
		if(targetId == 798959) 
		{
			...
			else if(qs != null && qs.getStatus() == QuestStatus.START && qs.getQuestVarById(0) == 1)
			{
				...
				{				
					if(!ItemService.addItems(player, Collections.singletonList(new QuestItems(182206724, 1))))
							return true;						
					qs.setQuestVarById(0, qs.getQuestVarById(0) + 1);
					updateQuestStatus(env);
					PacketSendUtility.sendPacket(player, new SM_DIALOG_WINDOW(env.getVisibleObject().getObjectId(), 10));
