...
public class _18602NightmareinShiningArmor extends QuestHandler
{
	...
	public HandlerResult onItemUseEvent(final QuestCookie env, final Item item)
	{
		...
		if (qs != null && qs.getStatus() == QuestStatus.COMPLETE || qs != null && qs.getStatus() == QuestStatus.START)
		{
			...
			{
				if (ItemService.canAddItems(player, Collections.singletonList(new QuestItems(164000140, 1))))
					ItemService.addItem(player, 164000140, 1, 60 * 7200);
			}
			if (id == 164000142 && !hasItem(player, 164000142))
			{
				if (ItemService.canAddItems(player, Collections.singletonList(new QuestItems(164000142, 1))))
					ItemService.addItem(player, 164000142, 1, 60 * 7200);
			}
			if (id == 164000143 && !hasItem(player, 164000143))
			{
				if (ItemService.canAddItems(player, Collections.singletonList(new QuestItems(164000143, 1))))
					ItemService.addItem(player, 164000143, 1, 60 * 7200);
			}
	...
	public boolean onActionItemEvent(QuestCookie env)
	{
		...
		return targetId == 730308 || targetId == 700939 || targetId == 730340 ||
			   targetId == 730341 || targetId == 730325 || targetId == 730338 ||
			   targetId == 730339 || targetId == 730337 || targetId == 730336;
	}
