...
public class MySQL5PlayerDAO extends PlayerDAO
{
	...
	public void storePlayer(final Player player)
	{
		...
		try
		{
			...
			PreparedStatement stmt = con.prepareStatement(
				"UPDATE players SET name=?, exp=?, recoverexp=?, x=?, y=?, z=?, heading=?, world_id=?, gender=?, player_class=?, last_online=?, cube_size=?, advanced_stigma_slot_size=?, warehouse_size=?, note=?, bind_point=?, title_id=?, mailboxLetters=?, repletionstate=?,  bg_points=?, personal_rating=?, arena_points=?, partner_id=? WHERE id=?");
			...
			log.debug("[DAO: MySQL5PlayerDAO] storing player "+player.getObjectId()+" "+player.getName());
				...
				stmt.setInt(20, player.getCommonData().getBattleGroundPoints());
				stmt.setInt(21, player.getCommonData().getPersonalRating());
				stmt.setInt(22, player.getCommonData().getArenaPoints());
                
                //Wedding
                if(player.getCommonData().getMarriage())
                {
                    stmt.setInt(23, getIdOverName(player.getCommonData().getPartner()));
                }else
                {
                    stmt.setNull(23, 0);
                }
	...
	public PlayerCommonData loadPlayerCommonData(final int playerObjId)
	{
		...
		try
		{
			...
			if (resultSet.next())
			{
				...
				cd.setArenaPoints(resultSet.getInt("arena_points"));
				cd.setPersonalRating(resultSet.getInt("personal_rating"));
				...
				String partner = getNameOverID(partnerID);
				if(partner != null)
				{
				    cd.setMarriage(true);
				    cd.setPartner(partner);
				}
	...
	@Override
	public Race getPlayerRaceByObjId(final int playerObjId)
	{
		Connection con = null;
		Race result = Race.NONE;
		try
		{
			con = DatabaseFactory.getConnection();
			PreparedStatement stmt = con.prepareStatement("SELECT race FROM players WHERE id = ?");
			stmt.setInt(1, playerObjId);
			ResultSet rs = stmt.executeQuery();
			if(rs.next())
				result = Race.valueOf(rs.getString("race"));
			else
				result = Race.NONE;
		}
		catch(Exception e)
	...
	public int getCharacterCountForRace(Race race)
	{
		...
		try
		{
			...
			if(GSConfig.FACTIONS_RATIO_LAST_ONLINE_ENABLE)
			{
				s = con.prepareStatement("SELECT COUNT(DISTINCT(`account_name`)) AS `count` FROM `players` WHERE `race` = ? AND `exp` >= ? AND `last_online` >= ?");
				
				Timestamp lastOnlineLimit = new Timestamp(System.currentTimeMillis() - (GSConfig.FACTIONS_RATIO_LAST_ONLINE_DAYS * 86400000L));
				s.setTimestamp(3, lastOnlineLimit);
			}
			else
				s = con.prepareStatement("SELECT COUNT(DISTINCT(`account_name`)) AS `count` FROM `players` WHERE `race` = ? AND `exp` >= ?");
    ...
    private String getNameOverID(int partnerID)
    {
        Connection con = null;
        String name = null;
        try 
        {
            con = DatabaseFactory.getConnection();
            PreparedStatement s = con.prepareStatement("SELECT name FROM `players` WHERE `id` = ?");
    ...
    private int getIdOverName(String partner)
    {
        Connection con = null;
        int id = 0;
        try 
