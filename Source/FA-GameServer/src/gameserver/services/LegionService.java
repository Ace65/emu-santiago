...
public class LegionService
{
	...
	public void disbandLegion(Legion legion)
	{
		...
		{
			LegionMemberEx exMember = getLegionMemberEx(memberObjId);
			if (exMember != null)
			{
				members.add(memberObjId);
				this.allCachedLegionMembers.remove(getLegionMemberEx(memberObjId));
				deleteLegionMemberFromDB(exMember);
			}
		}
		for(Integer memberObjId : members)
		{
			legion.deleteLegionMember(memberObjId);
		}
	...
	public void createLegion(Player activePlayer, String legionName)
	{
		...
		{
			if(!activePlayer.getInventory().decreaseKinah(LegionConfig.LEGION_CREATE_REQUIRED_KINAH))
				return;
			...
			RequestResponseHandler responseHandler = new RequestResponseHandler(activePlayer){
				...
				public void acceptRequest(Creature requester, Player responder)
				{
					...
					else
					{
						...
						if(legionMember.getRank().getRankId() > LegionRank.BRIGADE_GENERAL.getRankId() && legionRestrictions.canAppointBrigadeGeneral(activePlayer, targetPlayer))
						{
    ...
    private void appointRank(Player activePlayer, Player targetPlayer, int rank)
    {
        if(legionRestrictions.canAppointRank(activePlayer, targetPlayer))
        {
            ...
            int msgId;
            LegionMember legionMember = targetPlayer.getLegionMember();
            if(rank == LegionRank.DEPUTY.getRankId() && legionMember.getRank() != LegionRank.DEPUTY)
            {
                legionMember.setRank(LegionRank.DEPUTY);
                msgId = 1400902;
            }
	...
	private void requestChangeLevel(Player activePlayer, long kinahAmount)
	{
		...
		{
			Legion legion = activePlayer.getLegion();
			if(!activePlayer.getInventory().decreaseKinah(legion.getKinahPrice()))
				return;
	...
	private void updateMembersEmblem(Legion legion)
	{
		...
		for(Player onlineLegionMember : legion.getOnlineLegionMembers())
		{
//			if(true)//!legionEmblem.getIsCustom())
//			{
			PacketSendUtility.broadcastPacket(onlineLegionMember, new SM_LEGION_UPDATE_EMBLEM(legion.getLegionId(),
				legionEmblem.getEmblemVer(), legionEmblem.getColor_r(), legionEmblem.getColor_g(), legionEmblem
				.getColor_b(), legionEmblem.getIsCustom()),true);
//			}
//			else
//			{
//				sendCustomLegionPacket(onlineLegionMember, legionEmblem, legion.getLegionId(), legion.getLegionName());
//			}
		}
	}
	
	public void sendCustomLegionPacket(Player player, LegionEmblem emblem, int legionId, String legionName)
	{
		PacketSendUtility.sendPacket(player, new SM_LEGION_EMBLEM(legionId,  emblem.getEmblemVer(), emblem.getCustomEmblemData().length, emblem.getColor_r(), emblem.getColor_g(), emblem.getColor_b(), legionName, emblem.getIsCustom()));
		ByteBuffer buf = ByteBuffer.allocate(emblem.getCustomEmblemData().length);
		buf.put(emblem.getCustomEmblemData()).position(0);
		log.debug("Emblem Size: " + buf.capacity() + " bytes");
		int maxSize = 7993;
		int currentSize;
		byte[] bytes;
		do
		{
			log.debug("Emblem data position: byte " +  buf.position());			
			currentSize = buf.capacity()-buf.position();
			log.debug("Emblem data remaining: " + currentSize + " bytes");

			if ( currentSize >= maxSize)
			{
				bytes = new byte[maxSize];
				for (int i = 0 ; i < maxSize; i++) bytes[i] = buf.get(); 
				log.debug("SENDING Emblem Size: " + (bytes.length) + " bytes");
				PacketSendUtility.sendPacket(player, new SM_LEGION_EMBLEM_SEND(maxSize, bytes));

			}
			else
			{
				bytes = new byte[currentSize];
				for (int i = 0 ; i < currentSize; i++) bytes[i] = buf.get(); 
				log.debug("SENDING Emblem Size: " + (bytes.length) + " bytes");
				PacketSendUtility.sendPacket(player, new SM_LEGION_EMBLEM_SEND(currentSize,bytes));
			}
	...
	public void storeLegionEmblem(Player activePlayer, LegionEmblem customEmblem)
	{
		...
		if(!activePlayer.getInventory().decreaseKinah(LegionConfig.LEGION_EMBLEM_REQUIRED_KINAH))
			return;
	...
	public void storeLegionEmblem(Player activePlayer, int legionId, int emblemVer, int color_r, int color_g, int color_b, boolean isCustom)
	{
		if(legionRestrictions.canStoreLegionEmblem(activePlayer, legionId, emblemVer))
		{
			Legion legion = activePlayer.getLegion();
			...
			legion.getLegionEmblem().setEmblem(emblemVer, color_r, color_g, color_b, isCustom);
	...
	public void openLegionWarehouse(Player activePlayer)
	{
		...
		if(legionRestrictions.canOpenWarehouse(activePlayer))
		{
			...
			PacketSendUtility.sendPacket(activePlayer, new SM_WAREHOUSE_INFO(null,
				StorageType.LEGION_WAREHOUSE.getId(), expandLvl, false));
			
			legion.getLegionWarehouse().setUser(activePlayer.getObjectId());
			activePlayer.getObserveController().attach(
				new ActionObserver(ObserverType.MOVE)
					{
						@Override
						public void moved()
						{
							legion.getLegionWarehouse().setUser(0);
						}
					}
			);
	 ...
	 * @param totalSize
	 */
	public void uploadEmblemInfo(Player activePlayer, int totalSize, int red, int green, int blue)
	{
		if(legionRestrictions.canUploadEmblemInfo(activePlayer))
		{
			LegionEmblem legionEmblem = activePlayer.getLegion().getLegionEmblem();
			legionEmblem.setEmblem(legionEmblem.getIsCustom() ? (legionEmblem.getEmblemVer() + 1) : 0, red, green, blue, true);
	 ...
	 */
	private void storeNewAnnouncement(int legionId, Timestamp currentTime, String message)
	{
		DAOManager.getDAO(LegionDAO.class).saveNewAnnouncement(legionId, currentTime, message);
	}
	...
	private void addLegionMember(Legion legion, Player player, LegionRank rank)
	{
		...
		LegionEmblem legionEmblem = legion.getLegionEmblem();
		PacketSendUtility.broadcastPacket(player, new SM_LEGION_UPDATE_EMBLEM(legion.getLegionId(), legionEmblem
			.getEmblemVer(), legionEmblem.getColor_r(), legionEmblem.getColor_g(), legionEmblem.getColor_b(), legionEmblem.getIsCustom()), true);
	...
	private boolean removeLegionMember(String charName, boolean kick, String playerName)
	{
		...
		legionMember.getLegion().deleteLegionMember(legionMember.getObjectId());
		/**
...
// Send legion emblem: Note this is unneeded and breaks emblems
//		LegionEmblem emblem = legion.getLegionEmblem();
//		PacketSendUtility.sendPacket(activePlayer, new SM_LEGION_EMBLEM(legion.getLegionId(), emblem.getEmblemVer(), emblem.getColor_r(), emblem.getColor_g(), emblem.getColor_b(), legion.getLegionName(), emblem.getIsCustom()));
	...
	private class LegionRestrictions
	{
		...
		private boolean canInvitePlayer(Player activePlayer, Player targetPlayer)
		{
			...
			{
				PacketSendUtility.sendPacket(activePlayer, SM_SYSTEM_MESSAGE
					.STR_GUILD_INVITE_CAN_NOT_INVITE_OTHER_RACE());
				return false;
			}
			return true;
		}
		...
		private boolean canDisbandLegion(Player activePlayer, Legion legion)
		{
			...
			else if(legion.getLegionWarehouse().isInUse())
			{
			...
			else if(legion.getLegionWarehouse().getStorageItems().size() > 0 || legion.getLegionWarehouse().size() > 0)
			{
				PacketSendUtility.sendPacket(activePlayer, SM_SYSTEM_MESSAGE
		...
		private boolean canLeave(Player activePlayer)
		{
			...
			else if (activePlayer.getLegion() != null && activePlayer.getLegion().getLegionWarehouse().getUser() == activePlayer.getObjectId())
			{
		...
		public boolean canOpenWarehouse(Player activePlayer)
		{
			...
			{
				PacketSendUtility.sendPacket(activePlayer, SM_SYSTEM_MESSAGE
					.STR_NO_GUILD_TO_DEPOSIT());
				return false;
			}
			else
			{
				//possible exploits if enable - need more checks in cm_split_item
				...
				{
					PacketSendUtility.sendPacket(activePlayer, SM_SYSTEM_MESSAGE.STR_GUILD_WAREHOUSE_IN_USE());
					return false;
				}
		 ...
		 * @return true if name is valid, false overwise
		 */
		private boolean isValidAnnouncement(String announcement)
		{
			String text = announcement;
			text = text.replaceAll("\n","");
			text = text.replaceAll("\r","");
			return LegionConfig.ANNOUNCEMENT_PATTERN.matcher(text).matches();
		}
	}
