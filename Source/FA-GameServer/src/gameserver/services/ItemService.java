...
{ 
	...
	public static void splitItem (Player player, int itemObjId, long splitAmount, int slotNum, int sourceStorageType, int destinationStorageType)
	{
		...
		{
			log.warn("[CHEAT] Player : " + player.getName() + " trying to duplicate an item ! Item id : " + itemToSplit.getItemId() + ", name : " + itemToSplit.getName() + " IP: " + player.getClientConnection().getIP());
			return;
		}
	...
	public static void mergeItems (Player player, int sourceItemObjId, long itemAmount, int destinationObjId, int sourceStorageType, int destinationStorageType)
	{
		...
		int maxStackCount = destinationItem.getItemTemplate().getMaxStackCount();
		
		long availableToMergeCount = maxStackCount - destinationItem.getItemCount();
		
		if(availableToMergeCount < itemAmount)
			itemAmount = availableToMergeCount;
