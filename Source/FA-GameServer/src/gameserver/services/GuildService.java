...
public class GuildService
{
	...
	public void enterGuild(Player player, Npc npc)
	{
		...
		// Log Guild NPC's with no template
		if (guildTemplate == null)
		{
			PacketSendUtility.sendMessage(player, "This guild is not available yet.");
			return;
		}
	...
	public void exitGuild(Player player, Npc npc)
	{
		...
		if(playerGuildId == guildId)
		{
			...
			if(currentQuest != 0)
			{
				...
				if (QuestEngine.getInstance().deleteQuest(player, currentQuest))
	...
	public void sendDailyQuest(Player player, GuildTemplate guildTemplate)
	{
		...
		switch(guildId)
		{
			...
			case 13:
			case 23:
				startQuestId = chooseQuestByLvl(player, guildTemplate, 10, 30);
				break;
			case 14:
			case 24:
				if(playerLevel >= 40 && playerLevel <= 49)
