...
public class AccountService
{
	...
	/**
     * Set logout time for accountId
     * 
     * @param accountId
     * @return boolean
     */
	public static boolean setLastLogoutTime(int accountId){
        Connection con = null;
        try {
            log.warn("Setting logout time for player.");
            con = DatabaseFactory.getConnection();
            PreparedStatement stmt = con.prepareStatement("UPDATE " + CustomConfig.AIONSHOP_DB + ".account_data SET last_logout = " + System.currentTimeMillis() + " WHERE id = " + accountId);
            stmt.execute();
            stmt.close();
            con.close();
            return true;
        ...
        {
            log.error("Cannot store last logout attempt to database.", e);
            return false;
        }
        finally
        {
