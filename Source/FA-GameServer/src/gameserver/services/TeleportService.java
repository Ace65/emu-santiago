	...
	public static void moveToBindLocation(Player player, boolean useTeleport)
	{
			...
			
		{
			if(player.getCommonData().getRace() == Race.ASMODIANS)
				TeleportService.teleportTo(player, player.getWorldId(), player.getInstanceId(), 276, 289, 162, 0);
			else
				TeleportService.teleportTo(player, player.getWorldId(), player.getInstanceId(), 276, 183, 162, 0);
		}
	}
