...
public class InstanceService
{
    ...
    public static boolean canEnterSoloInstance(Player player, int instanceMapId, int instanceId)
    {
        if(player.getInstanceCD(instanceMapId) == null)
            return true;
        else
    ...
    public static void moveToEntryPoint(Player player, PortalTemplate portalTemplate, boolean useTeleport)
    {
        ...
        for(EntryPoint point : entryPoints)
        {
			...
			if(point.getRace().equals(player.getCommonData().getRace()))
            {
                entryPoint = point;
                break;
            }
	...
	public synchronized static WorldMapInstance createBattleGroundInstance(int worldId)
    {
        WorldMap map = World.getInstance().getWorldMap(worldId);
        int nextInstanceId = map.getNextInstanceId();
