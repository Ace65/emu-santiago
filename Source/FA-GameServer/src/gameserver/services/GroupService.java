...
public class GroupService
{
		...
		for(Player member : players)
		{
			if((highestLevel - member.getLevel()) < 10 || group.isMenteering(member)){
				if (!group.isMentoring(member) || players.size() == 1)
				{
					// Exp reward
					if (checkMentor)
						expReward = StatFunctions.calculateGroupExperienceReward(member.getLevel(), owner);
					if (group.isMentoring(member) && players.size() == 1)
						partyLvlSum = member.getLevel();	//Withouth this you can get an execption (divided by zero)
					long currentExp = member.getCommonData().getExp();
					long reward = (expReward * member.getLevel())/partyLvlSum;
					if (players.size() == 1)
					{
						reward *= member.getRates().getXpRate();
					} else
	...
	private void disbandGroup(PlayerGroup group)
	{
		...
		group.removePlayerFromGroup(group.getGroupLeader());
	...
	private void instanceSpecialRewardCheck(Monster owner, PlayerGroup group)
	{
		switch(owner.getWorldId())
		{
			...
			break;
			case 300220000:
				SplinterInstanceService.getInstance().onGroupReward(owner, group);
				break;
			case 300170000:
