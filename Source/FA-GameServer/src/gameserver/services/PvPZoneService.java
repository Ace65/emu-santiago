...
public class PvPZoneService
{
	...
	public static boolean Spawn(int ELYGATEnpcID, int ASMOGATEnpcID, int CANNONnpcID)
	{
		if (!opened)
		{
			...
			eventnpc[15] = (Npc) InstanceService.addNewSpawn(mapId, 1, 799219, (float) 686.955, (float) 465.914, (float) 1022.67, (byte) 57, true); //seller
			eventnpc[16] = (Npc) InstanceService.addNewSpawn(mapId, 1, 730207, (float) 482.904, (float) 540.082, (float) 1034.74, (byte) 53, true); //canon
			eventnpc[17] = (Npc) InstanceService.addNewSpawn(mapId, 1, 250146, (float) 411.274, (float) 544.287, (float) 1072.08, (byte) 91, true); //gate right
			eventnpc[18] = (Npc) InstanceService.addNewSpawn(mapId, 1, 250146, (float) 416.153, (float) 473.500, (float) 1072.08, (byte) 31, true); //gate left
			eventnpc[19] = (Npc) InstanceService.addNewSpawn(mapId, 1, 700554, (float) 624.570, (float) 541.108, (float) 936.094, (byte) 60, true); //chest
			eventnpc[20] = (Npc) InstanceService.addNewSpawn(mapId, 1, 700473, (float) 609.516, (float) 481.285, (float) 936.027, (byte) 28, true); //Pirate Ship Treasure Box
			eventnpc[21] = (Npc) InstanceService.addNewSpawn(mapId, 1, 700554, (float) 578.419, (float) 514.348, (float) 944.670, (byte) 88, true); //Pirate Ship Treasure Box
			eventnpc[22] = (Npc) InstanceService.addNewSpawn(mapId, 1, 700554, (float) 351.892, (float) 587.142, (float) 948.015, (byte) 77, true); //Pirate Ship Treasure Box
			eventnpc[23] = (Npc) InstanceService.addNewSpawn(mapId, 1, 700554, (float) 283.347, (float) 452.374, (float) 952.558, (byte) 90, true); //Pirate Ship Treasure Box
			eventnpc[24] = (Npc) InstanceService.addNewSpawn(mapId, 1, 700554, (float) 239.519, (float) 523.133, (float) 948.674, (byte) 80, true); //Pirate Ship Treasure Box
			eventnpc[25] = (Npc) InstanceService.addNewSpawn(mapId, 1, 700554, (float) 428.203, (float) 536.419, (float) 946.658, (byte) 39, true); //Pirate Ship Treasure Box
			eventnpc[26] = (Npc) InstanceService.addNewSpawn(mapId, 1, 700554, (float) 474.668, (float) 573.676, (float) 958.078, (byte) 114, true); //Pirate Ship Treasure Box
			eventnpc[27] = (Npc) InstanceService.addNewSpawn(mapId, 1, 700554, (float) 516.533, (float) 533.960, (float) 958.744, (byte) 6, true); //Pirate Ship Treasure Box
			eventnpc[28] = (Npc) InstanceService.addNewSpawn(mapId, 1, 281443, (float) 242.355, (float) 506.127, (float) 948.674, (byte) 119, true); //Divine Crusader
			...
			eventnpc[33] = (Npc) InstanceService.addNewSpawn(mapId, 1, 700554, (float) 457.228, (float) 511.480, (float) 952.556, (byte) 6, true);
