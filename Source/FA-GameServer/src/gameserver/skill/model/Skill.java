...
public class Skill
{
	...
	public void useSkill()
	{
		...
		if (skillType == SkillType.CAST && skillTemplate.getSkillId() != 1801)//only casted skills are affected except return
		{
			int currentStat = 0;
			if (effector instanceof Player)
	...
	public void endCast()
	{
		...
		if (triggerChainSkill || checkNonTargetAOE())
		{
    		...
    		if(CustomConfig.SKILL_CHAIN_TRIGGER){
            	...
            	int chainProb = skillTemplate.getChainSkillProb();
                if (chainProb != 0) {
                    if (Rnd.get(100) < chainProb || chainProb == 100 || skillTemplate.getSkillId() == 235 || skillTemplate.getSkillId() == 249 || skillTemplate.getSkillId() == 2031) {
					this.chainSuccess = true;
			}else{
    					this.chainSuccess = false;
                    }
