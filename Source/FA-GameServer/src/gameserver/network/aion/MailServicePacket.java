...
public abstract class MailServicePacket extends InventoryPacket
{
	...
	{
	    Mailbox box = player.getMailbox();
	    int totalLetters = box.size();
	    int unreadLetters = box.getUreadLettersCount();
	    
		writeC(buf, 6);
		writeH(buf, totalLetters);
		writeH(buf, unreadLetters);
		writeH(buf, 0);
		writeH(buf, 0);
		writeH(buf, letterId.length);
		
		for(int i = 0; i < letterId.length; i++)
		{
		    writeD(buf, letterId[i]);
		}
	}
}
