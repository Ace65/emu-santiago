...
public class SM_PLAYER_INFO extends AionServerPacket
{
	...
	protected void writeImpl(AionConnection con, ByteBuffer buf)
	{
		...
		final int raceId;
        	...
        	if(player.getArenaStatus() > 1 && con.getActivePlayer().getArenaStatus() > 1 && player.getArenaTeam() != con.getActivePlayer().getArenaTeam() && !player.getAdminNeutral() && !con.getActivePlayer().getAdminNeutral())
        		raceId = (con.getActivePlayer().getCommonData().getRace().getRaceId() == 0 ? 1 : 0);
        	else if(player.getAdminNeutral() || con.getActivePlayer().getAdminNeutral())
        ...
        {
            playerName += player.getName()+"\uE020"+player.getCommonData().getPartner();
        }
        else
        {
		...
		for(Player pj : player.getKnownList().getPlayers() ) {
		    PacketSendUtility.sendPacket(pj, new SM_PLAYER_MOTION(player.getNewEmotionList(), 7, player));
		}
		*/
	}
}
