...
public class CM_CASTSPELL extends AionClientPacket
{
	...
	protected void runImpl()
	{
		...
		{
			if (((((FortressGeneral)player.getTarget()).getObjectTemplate().getRace() == Race.GCHIEF_LIGHT)&&(player.getCommonData().getRace()==Race.ELYOS))
			||((((FortressGeneral)player.getTarget()).getObjectTemplate().getRace() == Race.GCHIEF_DARK)&&(player.getCommonData().getRace()==Race.ASMODIANS)))
				return;
		}
