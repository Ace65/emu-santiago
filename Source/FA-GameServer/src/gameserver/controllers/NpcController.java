...
public class NpcController extends CreatureController<Npc>
{
	...
	public void onDialogRequest(Player player)
	{
		...
		if((getOwner().getNpcId() == 730430)||(getOwner().getNpcId() == 730431)|| AcademyBootcampService.isAcademyBootcamp(getOwner().getWorldId()))
		{
	...
	public void onDialogSelect(int dialogId, final Player player, int questId)
	{
		...
		switch(dialogId)
		{
			...
			case 10000:
				...
				// Academy entrance
                else if((getOwner().getNpcId() == 730430)||(getOwner().getNpcId() == 730431))
                	AcademyBootcampService.getInstance().TeleportPlayerToCurrentStage(player);
				// Academy recordkeeper round request
                else if(AcademyBootcampService.isAcademyBootcamp(getOwner().getWorldId()))
	        	  	AcademyBootcampService.getInstance().onDialogSelect(player, getOwner());
				// Crucible Challenger entrance
                else if((getOwner().getNpcId() == 730461)||(getOwner().getNpcId() == 730462))
                	TeleportService.teleportTo(player, 300320000, 330f, 1651f, 95.5f, 1000);
				// Crucible Challenger recordkeeper round request
                // else if(CrucibleChallengeService.isCrucibleChallenge(getOwner().getWorldId()))
	        	//   	CrucibleChallengeService.getInstance().onDialogSelect(player, getOwner());
	...
	public void onAttack(final Creature creature, int skillId, TYPE type, int damage, int logId, AttackStatus status, boolean notifyAttackedObservers, boolean sendPacket)
	{
		...
		// Training dummies
		if(getOwner().getName().equals("training dummy"))
			damage = 0;
