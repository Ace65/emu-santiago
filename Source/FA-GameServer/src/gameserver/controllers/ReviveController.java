...
public class ReviveController
{
	...
	public void skillRevive(boolean cheatCheck)
	{
		...
		{
			Logger.getLogger(this.getClass()).info("[AUDIT]Player "+player.getName()+" sending fake CM_REVIVE: Player wasnt skill resurrected!" + " IP: " + player.getClientConnection().getIP());
			return;
		}
		revive(10, 10);
	...
	public void instanceEntryRevive()
	{
		...
		if(player.isInInstance())
		{
			...
			{
				AcademyBootcampService.getInstance().TeleportToReadyRoom(player);
			}
			else
			{
	...
	public void battleGroundRevive()
	{
    	revive(100, 100);
	...
	public void AionArenaRevive()
	{
    	...
    	ArenaManager.teleport(player, player.getWorldId(), player.getInstanceId());
