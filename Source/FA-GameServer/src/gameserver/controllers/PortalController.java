...
public class PortalController extends NpcController
{
		...
		ThreadPoolManager.getInstance().schedule(new Runnable(){
			...
			private void analyzePortation(final Player player)
			{
				...
				for (EntryPoint point : portalTemplate.getEntryPoint()) {
					if (point.getRace() != null )
					{
						if(point.getRace().equals(player.getCommonData().getRace())) match = true;
					}
					else
						match = true;
				}
	...
	protected boolean checkInstanceCooldown(Player player, int worldId, int instanceId)
	{
		...
		{
            //zer0patches, only enable cooldowns for fortress instances
            boolean noCoolDown = true;
            switch(worldId){
                case 300050000: //AstreiaChamber
                case 300060000: //SulfurTreeNest    
                case 300070000: //ChamberOfRoah    
                case 300080000: //LeftWingChamber    
                case 300090000: //RightWingChamber    
                case 300120000: //KysisChamber    
                case 300130000: //MirenChamber    
                case 300140000: //KrotanChamber    
                case 300220000: //Abyssal Splinter
                    noCoolDown = false;
                    break;
            }
            if (noCoolDown == true) return false;
