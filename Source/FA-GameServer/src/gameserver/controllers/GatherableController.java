...
public class GatherableController extends VisibleObjectController<Gatherable>
{
	...
	public void onStartUse(final Player player)
	{
		...
		if (player.getEquipment().getEquippedItemsByItemId(122001250).size() != 0) // Check for ring's essencetapping equipped
		{
			// Check material to extract. This ring only affects to Ruby, Sapphire or Diamond
			if(template.getTemplateId() == 400310 || template.getTemplateId() == 400311 || template.getTemplateId() == 400312)
				factor = 1000000;
		}
		
		if (player.getEquipment().getEquippedItemsByItemId(122001251).size() != 0) // Check for ring's aethertapping equipped
		{
			// Check material to extract. This ring only affects to Burning Vortex
			if(template.getTemplateId() == 401021 || template.getTemplateId() == 401022)
				factor = 1000000;
		}
		...
		for(Material mat : materials)
		{
			...
			matTemp = mat;
			if( (Rnd.get(percent) + factor) <= rate)
			{
				material = mat;
				break;
			}
