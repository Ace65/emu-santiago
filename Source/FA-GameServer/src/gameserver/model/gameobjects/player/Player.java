...
import br.focus.utils.PlayerActionList;
...
public class Player extends Creature {
    ...
    public static final int                     CHAT_FIXED_ON_BOTH          = CHAT_FIXED_ON_ELYOS
                                                                                    | CHAT_FIXED_ON_ASMOS;
	...
	public int     ArenaStatus = 0;
	public int     ArenaTeam = 0;
    public int     ArenaTeamSessionPoints = 0;
    public int     ArenaTeamSessionKills = 0;
    public int     ArenaTeamSessionDeaths = 0;
	...
	public int SurveyCounter = 0;
	private AionArenaTeam arenaTeam2v2,arenaTeam3v3,arenaTeam5v5 = null;
	public int HideBuffStackCount = 0;

    /**
     * Academy Bootcamp points reward
     */
    private int academyBootcampPoints = 0;

    public Player(PlayerController controller, PlayerCommonData plCommonData,
            PlayerAppearance appereance, Account account) {
        super(plCommonData.getPlayerObjId(), controller, null, null,
        ...
        this.lastNpcDrops = new LastUsedCache<Integer, NpcDropStat>(
                DropConfig.NPC_DROP_HISTORY_COUNT);
    ...
    public NpcDropStat getNpcDropStats(int npcId) {
        ...
        lastNpcDrops.put(npcId, stats,
                DropConfig.NPC_DROP_EXPIRE_MINUTES * 60 * 1000);
        return stats;
    }
    ...
    public List<Item> getDirtyItemsToUpdate() {
        ...
        Storage regularWhStorage = getStorage(StorageType.REGULAR_WAREHOUSE
                .getId());
        ...
        Storage accountWhStorage = getStorage(StorageType.ACCOUNT_WAREHOUSE
                .getId());
    ...
    public void setCubesize(int cubesize) {
        ...
        getInventory().setLimit(
                getInventory().getLimit() + (cubesize * CUBE_SPACE));
    }
    ...
    /**
     * <b><font color='red'>NOTICE: </font>this method is supposed to be called
     * only from {@link PlayerService#playerLoggedIn(Player)}</b>
     */
    public void onLoggedIn() {
        getController().addTask(
                TaskId.PLAYER_UPDATE,
                ThreadPoolManager.getInstance().scheduleAtFixedRate(
                        new GeneralUpdateTask(this),
                        PeriodicSaveConfig.PLAYER_GENERAL * 1000,
                        PeriodicSaveConfig.PLAYER_GENERAL * 1000));
        getController().addTask(
                TaskId.INVENTORY_UPDATE,
                ThreadPoolManager.getInstance().scheduleAtFixedRate(
                        new ItemUpdateTask(this),
    ...
    /**
     * <b><font color='red'>NOTICE: </font>this method is supposed to be called
     * only from {@link PlayerService#playerLoggedOut(Player)}</b>
     */
    public void onLoggedOut() {
		...
		if(battleGround != null)
		{
			...
			battleGround.broadcastToBattleGround(getName() + " has left the battleground.", getCommonData().getRace());
			battleGround = null;
		...
		else if(ArenaStatus == 1)
		{
			ArenaManager.unregisterTeam(this);
		}
    }
    ...
    public void setWarehouseSize(int warehouseSize) {
        ...
        getWarehouse().setLimit(
                getWarehouse().getLimit() + (warehouseSize * WAREHOUSE_SPACE));
    }
    ...
    public boolean isUsingFlyTeleport() {
        return isInState(CreatureState.FLIGHT_TELEPORT)
                && flightTeleportId != 0;
    }
    ...
    public boolean isEnemyNpc(Npc npc) {
        if (npc.getObjectTemplate().getNpcType() == NpcType.NEUTRAL
                || npc.getObjectTemplate().getNpcType() == NpcType.ARTIFACT)
            return false;
        if (npc instanceof Monster)
            return true;
        else if (npc instanceof SkillAreaNpc || npc instanceof Homing
                || npc instanceof GroupGate)
            return false;
        // Trap, Totem, Servant
    ...
    public boolean isEnemyPlayer(Player player) {
        return (player.getCommonData().getRace() != getCommonData().getRace()
                || getController().isDueling(player) || ArenaService
                .getInstance().isEnemy(getController().getOwner(), player) || (player.getArenaStatus() > 1 && getArenaStatus() > 1 && player.getArenaTeam() != getArenaTeam()))
    ...
    public boolean isFriend(Player player) {
        return (player.getCommonData().getRace() == getCommonData().getRace() && !getController()
                .isDueling(player)) || player.getAdminNeutral();
    }
            ...
            case ELYOS:
                if (DataManager.TRIBE_RELATIONS_DATA.isGuardDark(npcTribe)
                        || DataManager.TRIBE_RELATIONS_DATA
                                .isGuardDrakan(npcTribe))
                    return true;
                return DataManager.TRIBE_RELATIONS_DATA.isAggressiveRelation(
                        npcTribe, "PC");
            case ASMODIANS:
    ...
    public void addItemCoolDown(int delayId, long time, int useDelay) {
        if (itemCoolDowns == null)
            itemCoolDowns = Collections
                    .synchronizedMap(new HashMap<Integer, ItemCooldown>());
    ...
    private class GeneralUpdateTask implements Runnable {
        ...
        public void run() {
            try {
                ...
                log.error("Exception during periodic saving of player "
                        + player.getName() + " " + ex.getCause() != null ? ex
    ...
    private class ItemUpdateTask implements Runnable {
        ...
        public void run() {
            try {
                ...
                log.error("Exception during periodic saving of player items "
                        + player.getName() + " " + ex.getCause() != null ? ex
                        .getCause().getMessage() : "null");
            }
        }
    ...
    public void setTvt(TvtController newTvt) {
        // TODO Auto-generated method stub
        this.tvt = newTvt;
    }

    public TvtController getTvt() {
        return this.tvt;
    }
        ...
        switch (chanId) {
            case CHAT_FIXED_ON_ASMOS:
                return LanguageHandler
                        .translate(CustomMessageId.CHANNEL_NAME_ASMOS);
            case CHAT_FIXED_ON_ELYOS:
                return LanguageHandler
                        .translate(CustomMessageId.CHANNEL_NAME_ELYOS);
            case CHAT_FIXED_ON_WORLD:
                return LanguageHandler
                        .translate(CustomMessageId.CHANNEL_NAME_WORLD);
            case CHAT_FIXED_ON_BOTH:
                return LanguageHandler
                        .translate(CustomMessageId.CHANNEL_NAME_BOTH);
        }
        return "";
    }
    ...
    public static String getChanCommand(int chanId) {
        switch (chanId) {
            case CHAT_FIXED_ON_ASMOS:
                return "."
                        + LanguageHandler
                                .translate(CustomMessageId.CHANNEL_COMMAND_ASMOS);
            case CHAT_FIXED_ON_ELYOS:
                return "."
                        + LanguageHandler
                                .translate(CustomMessageId.CHANNEL_COMMAND_ELYOS);
            case CHAT_FIXED_ON_WORLD:
                return "."
                        + LanguageHandler
                                .translate(CustomMessageId.CHANNEL_COMMAND_WORLD);
            case CHAT_FIXED_ON_BOTH:
                return "."
                        + LanguageHandler
                                .translate(CustomMessageId.CHANNEL_COMMAND_BOTH);
        }
        return "";
    }
    ...
    public void setBannedFromWorld(String by, String reason, long duration,
            Date date) {
    ...
    public boolean banFromWorld(String by, String reason, long duration) {
        if (isBannedFromWorld()) {
            ...
            if (!dao.addWorldBan(getObjectId(), by, bannedFromWorldDuring,
                    bannedFromWorldDate, bannedFromWorldReason)) {
                return false;
            }
    ...
    public void scheduleUnbanFromWorld() {
        if (!isBannedFromWorld()) {
            throw new RuntimeException(
                    "scheduling unban task when not banned from "
        ...
        if (time > 0) {
            final Date endDate = new Date(bannedFromWorldDate.getTime()
                    + bannedFromWorldDuring);
            taskToUnbanFromWorld = ThreadPoolManager.getInstance().schedule(
                    new Runnable() {
                        public void run() {
                            ...
                            if (endDate.getTime() <= Calendar.getInstance()
                                    .getTimeInMillis()) {
                                DAOManager.getDAO(PlayerWorldBanDAO.class)
                                        .removeWorldBan(playerObjId);
                            }
                            if (player != null) {
                                player.bannedFromWorld = false;
                                PacketSendUtility.sendSysMessage(
                                        player,
                                        LanguageHandler
    ...
    public String getBannedFromWorldRemainingTime() {
        ...
        if (bannedFromWorldDuring == 0) {
            ...
            elapsed = bannedFromWorldDuring
                    - (Calendar.getInstance().getTimeInMillis() - bannedFromWorldDate
                            .getTime());
     ...
     * @param instanceId
     */
    public void addInstanceCD(int instanceMapId, Timestamp CDEnd,
            int instanceId, int groupId) {
        InstanceCDs.put(instanceMapId, new InstanceCD(CDEnd, instanceId,
                groupId));
    }
    ...
    public boolean canSayInChannel() {
        if (System.currentTimeMillis() - lastMessageTime < CustomConfig.CHANNEL_MESSAGE_INTERVAL * 1000) {
            PacketSendUtility.sendMessage(this,
                    "You can't send messages often than one message in "
                            + CustomConfig.CHANNEL_MESSAGE_INTERVAL
                            + " seconds");
            return false;
        }
        return true;
    ...
    /**
     * @param setActiveMotion
     */
    public void setActiveMotion(int state, boolean active) {
        if(getMotionList().canAdd(state) == false)
            getMotionList().get(state).setActive(active);
    }
    
    /**
     * @param getActiveMotion
     */
    public boolean getActiveMotion(int state) {
        if(getMotionList().canAdd(state))
            return false;
        else
            return getMotionList().get(state).getActive();
    }

    /**
     * @param currentWaitingMotion
     *            the currentWaitingMotion to set
     */
    public void setCurrentWaitingMotion(int currentWaitingMotion) { 
        switch (currentWaitingMotion){
            case 0:
                setActiveMotion(1, false);
                setActiveMotion(5, false);                
                break;
                
            case 1:
                setActiveMotion(1, true);
                setActiveMotion(5, false);                 
                break;
            case 5:
                setActiveMotion(1, false);
                setActiveMotion(5, true);               
                break;
        } 
    }

    /**
     * @return the currentWaitingMotion
     */
    public int getCurrentWaitingMotion() {
        int state = 0;
        if(getActiveMotion(1))
            state = 1;
        else if(getActiveMotion(5))
            state = 5;
    ...
    /**
     * @param currentRunningMotion
     *            the currentRunningMotion to set
     */
    public void setCurrentRunningMotion(int currentRunningMotion) { 
        switch (currentRunningMotion){
            case 0:
                setActiveMotion(2, false);
                setActiveMotion(6, false);                
                break;
                
            case 2:
                setActiveMotion(2, true);
                setActiveMotion(6, false);                 
                break;
            case 6:
                setActiveMotion(2, false);
                setActiveMotion(6, true);               
                break;
        }   
    }

    /**
     * @return the currentRunningMotion
     */
    public int getCurrentRunningMotion() {
        int state = 0;
        if(getActiveMotion(2))
            state = 2;
        else if(getActiveMotion(6))
            state = 6;
    ...
    /**
     * @param currentJumpingMotion
     *            the currentJumpingMotion to set
     */
    public void setCurrentJumpingMotion(int currentJumpingMotion) {
        switch (currentJumpingMotion){
            case 0:
                setActiveMotion(3, false);
                setActiveMotion(7, false);                
                break;
                
            case 3:
                setActiveMotion(3, true);
                setActiveMotion(7, false);                 
                break;
            case 7:
                setActiveMotion(3, false);
                setActiveMotion(7, true);               
                break;
        }  
    }

    /**
     * @return the currentJumpingMotion
     */
    public int getCurrentJumpingMotion() {
        int state = 0;
        if(getActiveMotion(3))
            state = 3;
        else if(getActiveMotion(7))
            state = 7;
    ...
    /**
     * @param currentRestMotion
     *            the currentRestMotion to set
     */
    public void setCurrentRestMotion(int currentRestMotion) {
        switch (currentRestMotion){
            case 0:
                setActiveMotion(4, false);
                setActiveMotion(8, false);                
                break;
                
            case 4:
                setActiveMotion(4, true);
                setActiveMotion(8, false);                 
                break;
            case 8:
                setActiveMotion(4, false);
                setActiveMotion(8, true);               
                break;
        }  
    }

    /**
     * @return the currentRestMotion
     */
    public int getCurrentRestMotion() {
        int state = 0;
        if(getActiveMotion(4))
            state = 4;
        else if(getActiveMotion(8))
            state = 8;
	...
	/**
	* @return the arenaTeam2v2
	*/
	public AionArenaTeam getArenaTeam2v2()
	{
		return arenaTeam2v2;
	}

	/**
	* @param arenaTeam2v2 the arenaTeam2v2 to set
	*/
	public void setArenaTeam2v2(AionArenaTeam arenaTeam2v2)
	{
		this.arenaTeam2v2 = arenaTeam2v2;
	}

	/**
	* @return the arenaTeam3v3
	*/
	public AionArenaTeam getArenaTeam3v3()
	{
		return arenaTeam3v3;
	}

	/**
	* @param arenaTeam3v3 the arenaTeam3v3 to set
	*/
	public void setArenaTeam3v3(AionArenaTeam arenaTeam3v3)
	{
		this.arenaTeam3v3 = arenaTeam3v3;
	}
	
	/**
	* @return the arenaTeam5v5
	*/
	public AionArenaTeam getArenaTeam5v5()
	{
		return arenaTeam5v5;
	}

	/**
	* @param arenaTeam5v5 the arenaTeam5v5 to set
	*/
	public void setArenaTeam5v5(AionArenaTeam arenaTeam5v5)
	{
		this.arenaTeam5v5 = arenaTeam5v5;
    }
	...
	public int getArenaTeam()
	{
		return ArenaTeam;
	}

    /**
     * @param Reward points to be set on stage win
     */
    public void setAcademyBootcampRewardPoints(int points)
    {
    	this.academyBootcampPoints = points;
    }
    
    /**
     * @return Current Bootcamp points
     */
    public int getAcademyBootcampRewardPoints()
    {
    	return this.academyBootcampPoints;
    }
}