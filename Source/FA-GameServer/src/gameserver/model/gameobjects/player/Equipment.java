...
public class Equipment
{
	...
	public void onLoadHandler(Item item)
	{
		...
		{
			log.warn("Duplicate equipped item in slot : " + item.getEquipmentSlot() + " Player:" + owner.getName() + " IP: " + owner.getClientConnection().getIP() );
			return;
		}
		...
		if (!item.getItemTemplate().checkClassRestrict(owner.getPlayerClass()))
			log.info("[AUDIT]Player: "+owner.getName()+" has equipped item: "+item.getObjectId()+" which is unequiptable by its class!" + " IP: " + owner.getClientConnection().getIP());
