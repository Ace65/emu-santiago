...
public class PlayerGameStats extends CreatureGameStats<Player>
{
	...
	public void recomputeStats()
	{
		try
		{
			...
			synchronized (statsModifiers)
			{
		        ...
		        for (StatEffectId eid : statEffectIds)
		        {
					...
					for(StatModifier modifier : modifiers)
					{
						...
						{
							StatEnum statEnumToModify = modifier.getStat().getMainOrSubHandStat(slot, magicalAttack);
							
							//filter out sub hand modifiers
							if (owner.getEquipment().isDualWieldEquipped())
							{
								if (slot == ItemSlot.SUB_HAND && (statEnumToModify == StatEnum.PARRY || statEnumToModify == 
								    StatEnum.MAGICAL_ACCURACY) && !modifier.isBonus())
								{
								...
								if ((slot == ItemSlot.SUB_HAND || slot == ItemSlot.MAIN_HAND) && statEnumToModify == StatEnum.ATTACK_SPEED)
								{
									if (modifier.isBonus())
									{
									...
									else if (slot == ItemSlot.MAIN_HAND)
									{
										if (attackSpeedApplied)
							...
							if (statEnumToModify == StatEnum.MAIN_HAND_PHYSICAL_ATTACK && !modifier.isBonus())
							{
							...
							if(!orderedModifiers.containsKey(statEnumToModify))
							{
								orderedModifiers.put(statEnumToModify, new StatModifiers());
							}
							
							orderedModifiers.get(statEnumToModify).add(modifier);
						}
					}
				}
			}
			...
			if(aspeedBonuses.size() > 0) {
    			StatModifier aspeedBonus = aspeedBonuses.get(0);
    			for (int i = 1; i < aspeedBonuses.size(); i++)
    			{
    			    SimpleModifier mod = (SimpleModifier)aspeedBonuses.get(i);
    			    if (((SimpleModifier)aspeedBonus).getValue() > mod.getValue())
    					aspeedBonus = mod;
    			}
