...
{	
	...
	public boolean canAffectBySkill(Player player, VisibleObject target)
	{
		...
		if (target instanceof Player)
		{
			...
			if (targetPlayer.getAdminNeutral() && targetPlayer != player)
				return false;
		}
		
		if(player.ArenaStatus == 2 && target instanceof Player && ((Player)target).ArenaStatus == 2 && player.ArenaTeam != ((Player)target).ArenaTeam)
			return false;
		...
		if(player.battlegroundObserve >= 2)
            return false;
