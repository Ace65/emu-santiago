...
import gameserver.controllers.BattleGroundHealerController;
import gameserver.controllers.BindpointController;
import gameserver.controllers.ChestController;
import gameserver.controllers.weaponSiegeController;
...
public class SpawnEngine
{
	...
	*/
	...
	public BattleGroundHealer spawnBGHealer(SpawnTemplate spawn, int instanceId, Race race)
    {
    ...
    public BattleGroundFlag spawnBGFlag(SpawnTemplate spawn, int instanceId, Race race)
    {
    ...
    public BattleGroundAgent spawnBGAgent(SpawnTemplate spawn, int instanceId, int npcId)
    {
	 ...
	 * @return npc
	 */
	public Npc spawnWeaponSiege(SpawnTemplate spawn, int instanceIndex)
	{
		int objectId = spawn.getSpawnGroup().getNpcid();
		NpcTemplate npcTemplate = DataManager.NPC_DATA.getNpcTemplate(objectId);
		Npc npc = new Npc(IDFactory.getInstance().nextId(), new weaponSiegeController(), spawn,
			npcTemplate);
		npc.setKnownlist(new NpcKnownList(npc));
		npc.setNpcSkillList(DataManager.NPC_SKILL_DATA.getNpcSkillList(npcTemplate.getTemplateId()));
		npc.setEffectController(new EffectController(npc));
		npc.getController().onRespawn();
		bringIntoWorld(npc, spawn, instanceIndex);
		return npc;
