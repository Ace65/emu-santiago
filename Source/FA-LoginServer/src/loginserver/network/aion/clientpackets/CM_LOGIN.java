...
public class CM_LOGIN extends AionClientPacket
{
	...
	private boolean lastLogoutTimeOK(AionConnection client){
        log.warn("Checking if player logged out less than 15 seconds ago.");
        long last_logout_time = DAOManager.getDAO(AccountDAO.class).getLastLogout(client.getAccount().getId());
        if ((System.currentTimeMillis() - last_logout_time) < 15000){
            return false;
        }
        return true;
	}
